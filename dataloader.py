...
def load_graph_dataset(dataset_name, device, re_split=False, path_prefix="../.."):
    graph_data = torch.load(f"{path_prefix}/datasets/{dataset_name}.pt", weights_only=False, map_location='cpu').to(
        device)
    # Alternative 
...
def load_graph_dataset_for_gnn(dataset_name, device, re_split=False, path_prefix="../..", emb_model="shallow"):
    ...
    if emb_model != "shallow":
        ...
        node_feat = torch.load(f"{path_prefix}/datasets/{emb_model}/{dataset_name}.pt", map_location=device,
                               weights_only=False).to(device).type(torch.float)
