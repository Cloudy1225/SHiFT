...
{{
  ...
  "concept_split_needed": true or false
    ...
    def _refine_concepts(self, candidate_concepts: List[Dict[str, Any]],
        ...
        user_prompt = f"""Context: Under the parent category {parent_info}, we identified {len(candidate_concepts)} candidate sub-concepts based on clustering. However, the clustering algorithm may have produced:
1. Duplicates/Synonyms (e.g., "GNN" and "Graph Neural Nets" should be MERGED).
2. Overly fragmented topics (e.g., "LSTM applications" and "RNN models" might be MERGED into "Recurrent Neural Networks").
3. Outliers/Irrelevant topics (e.g., a Biology topic appearing in a CS list should be PRUNED).

Candidate Sub-concepts:
ID | Name | Definition
{concepts_str}

Task: Review the list and output a set of operations to refine this level of the taxonomy.
Allowed Operations:
- MERGE: Combine IDs [id1, id2, ...] into a new, broader concept. Provide a new Name and Definition.
...
[
  {{"op": "MERGE", "source_ids": [1, 5, 8], "new_name": "Graph Neural Networks", "new_def": "...", "reason": "Synonyms"}},
  {{"op": "PRUNE", "target_id": 3, "reason": "Off-topic physics paper"}},
  {{"op": "KEEP", "target_id": 2, "new_name": "Transformers"}}
]"""
